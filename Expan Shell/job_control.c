...
job * new_job(pid_t pid,int veces, int espiado,const char * command,char ** commando, enum job_state state)
{
	job * aux;
...
void print_item(job * item)
{
	...
	char cadena[100];
	strcpy(cadena," ");
	//Veo si esta espiado o no
	if(item->espiado==1){
		strcpy(cadena,"[spy]");
	}
	
	printf("pid: %d,veces: %d command: %s, state: %s   %s\n", item->pgid,item->veces, item->command, state_strings[item->state],cadena);
}
